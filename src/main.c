...
void calculateMap() {
    ...
    for (i = 0; i < MAX_STARPORTS; i++) {
        if (i != cur_starport) {
            ...
            /*
                To show the distance in parsecs instead of light-years,
                divide the distance calculated by 3.26156.
            */
...
void inspectShip() {
    wclear(shipstatus);
    wbkgd(shipstatus,COLOR_PAIR(CYAN_PAIR));
    ...
    wattrset(shipstatus,A_REVERSE | A_BOLD);
...
void renewLease() {
    int key;
    wclear(win);
    wbkgd(win,COLOR_PAIR(BLUE_PAIR));
    box(win,ACS_VLINE,ACS_HLINE);
    wattrset(win,A_BOLD);
...
void newGame() {
    ...
    wresize(win,height,width);
    box(win,ACS_VLINE,ACS_HLINE);
    wbkgd(win,COLOR_PAIR(BLUE_PAIR) | A_DIM);
